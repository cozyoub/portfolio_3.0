...
function buildLinkInfo(work) {
  ...
  if (work.solutionTag) {
    list.push({ label: '기술 스택', value: work.solutionTag });
  }
  if (work.scopeTags?.length) {
    list.push({ label: '작업 범위', value: work.scopeTags.map((s) => s.label).join(', ') });
  }
  if (work.contributionPercent != null) {
    list.push({ label: '작업 기여도', value: `${work.contributionPercent}%` });
  }
  if (work.contribution) {
    list.push({ label: '기여도 상세', value: work.contribution });
  }
  if (work.techStack) {
  ...
  if (work.githubUrl) {
    if (Array.isArray(work.githubUrl)) {
      work.githubUrl.forEach((gh) => {
        list.push({
          label: `깃허브${gh.label ? ` (${gh.label})` : ''}`,
          href: gh.url,
          value: gh.label ? `${gh.label} 저장소` : '깃허브 URL',
        });
      });
    } else {
...
function WorkDetailPage() {
  ...
  const records = buildRecords(work);
